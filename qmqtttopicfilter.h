...
size_t qHash(const QMqttTopicFilter &name, size_t seed = 0) Q_DECL_NOTHROW;
...
public:
    ...
    friend bool operator==(const QMqttTopicFilter &lhs, const QMqttTopicFilter &rhs) Q_DECL_NOTHROW;
    friend inline bool operator!=(const QMqttTopicFilter &lhs, const QMqttTopicFilter &rhs) Q_DECL_NOTHROW { return !(lhs == rhs); }
    friend bool operator<(const QMqttTopicFilter &lhs, const QMqttTopicFilter &rhs) Q_DECL_NOTHROW;
    friend size_t qHash(const QMqttTopicFilter &filter, size_t seed) Q_DECL_NOTHROW;
...
QDataStream &operator<<(QDataStream &, const QMqttTopicFilter &);
QDataStream &operator>>(QDataStream &, QMqttTopicFilter &);
#endif

#ifndef QT_NO_DEBUG_STREAM
QDebug operator<<(QDebug, const QMqttTopicFilter &);
#endif
