...
size_t qHash(const QMqttTopicName &name, size_t seed = 0) Q_DECL_NOTHROW;
...
public:
    ...
    friend bool operator==(const QMqttTopicName &lhs, const QMqttTopicName &rhs) Q_DECL_NOTHROW;
    friend inline bool operator!=(const QMqttTopicName &lhs, const QMqttTopicName &rhs) Q_DECL_NOTHROW { return !(lhs == rhs); }
    friend bool operator<(const QMqttTopicName &lhs, const QMqttTopicName &rhs) Q_DECL_NOTHROW;
    friend size_t qHash(const QMqttTopicName &name, size_t seed) Q_DECL_NOTHROW;
...
QDataStream &operator<<(QDataStream &, const QMqttTopicName &);
QDataStream &operator>>(QDataStream &, QMqttTopicName &);
#endif

#ifndef QT_NO_DEBUG_STREAM
QDebug operator<<(QDebug, const QMqttTopicName &);
#endif
